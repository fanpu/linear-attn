...
| riddled / intermingled basins? | **No.** f(ε) keeps falling all the way to ε = 10⁻¹³ in both windows; riddling would make it flat (α ≈ 0). The fringe window does show a shallower tail (α ≈ 0.2 for ε < 10⁻⁵), so the boundary dimension there approaches 1.8: the basins are close to intermingled but not riddled. See RIDDLE_PLACEHOLDER for the 10⁶-sample tail. |
