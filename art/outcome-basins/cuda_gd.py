...
__global__ void mlp_g(int64_t off, int64_t P, int H, int N, const double* X, const double* Y, const double* th0, double* th,
                    double eta, int T, double tol, double thr, double* tconv, int8_t* status, double* lossv) {
    int64_t p = off + (int64_t)blockIdx.x * BS + threadIdx.x; if (p >= P) return;
    int D = 4*H + 1;
...
MLP_L = ('mlp_g' + LAUNCH + 'nb, BS>>>(off, std::min(off + n, P), (int)H, N, X.data_ptr<double>(), Y.data_ptr<double>(), th0.data_ptr<double>(), '
         'th.data_ptr<double>(), eta, (int)T, tol, thr, tconv.data_ptr<double>(), status.data_ptr<int8_t>(), loss.data_ptr<double>()); torch::cuda::synchronize();')
CPP_SRC = r'''
#include <torch/extension.h>
...
void mlp_run(torch::Tensor X, torch::Tensor Y, int64_t H, torch::Tensor th0, torch::Tensor th, double eta, int64_t T, double tol, double thr, torch::Tensor tconv, torch::Tensor status, torch::Tensor loss, int64_t per);
'''
...
def module():
    ...
    if _mod is None:
        ...
        _mod = load_inline('outcome_gd', cpp_sources=CPP_SRC, cuda_sources=src,
...
def run(kind, th0, eta, T, tol=1e-12, thr=1e4, per=1 << 16, X=None, Y=None, H=None, chunk=1 << 22):
    ...
    for s in range(0, P, chunk):
        ...
        if kind == 'fact':
            ...
            m.mlp_run(Xt, Yt, int(H), a, th, float(eta), int(T), float(tol), float(thr), tc, st, lo, int(per))
        e = s + a.shape[0]
