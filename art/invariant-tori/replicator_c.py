"""COPIED from art/game-chaos/replicator_c.py (do not edit the source piece). Local change: the
trajectory buffer stores the float64 logits (u1,u2,v1,v2) instead of probabilities, so orbits
stay exact for the chart and chained runs are bit-identical; the rest is unchanged.
...
/*
 ...
 traj_every: store logits (u1,u2,v1,v2) every traj_every steps into traj (n, ntraj, 4).
    ...
    for (int o = 0; o < n; o++) {
        ...
        for (long t = 1; t <= nsteps; t++) {
            ...
            if (traj_every > 0 && t % traj_every == 0 && ti < ntraj) {
                memcpy(traj + 4*((long)o*ntraj+ti), s, 4*sizeof(double)); ti++;
            }
...
def integrate(s0, ex, ey, h=0.01, T=1000.0, every=100, traj_every=0, ntraj=0, maxsec=0,
    ...
    traj = np.zeros((n, max(ntraj, 1), 4)); sec = np.zeros((n, max(maxsec, 1), 6))
