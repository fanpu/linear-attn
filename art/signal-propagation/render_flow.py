...
def colour(c, style):
    ...
    g = np.clip((np.log10(np.clip(1 - c, 1e-4, 1)) + 4) / 4, 0, 1)
    if style == "oslo":
        return plt.get_cmap("cmc.oslo")(g)[..., :3]
    if style == "cyanotype":
        return P.get("cyanotype")(1 - g)[..., :3]
    if style == "spectral":
        # declared: split by the measured phase; shade = log10(1 - c) on [-4, 0], oriented so the two dark
        # ends meet where the pair is least settled (ordered: not yet merged; chaotic: c* close to 1)
        # per-side rank of log10(1 - c) (Sohl-Dickstein-style CDF normalisation, per frame); pairs merged
        # below 1 - c = 1e-4 sit at the pale end
        from scipy.stats import rankdata
        v = np.log10(np.clip(1 - c, 1e-4, 1)); u = np.zeros_like(v)
        for m, flip in ((ch, False), (~ch, True)):
            live = m & (v > -4)
            if live.any():
                r = (rankdata(v[live]) - 0.5) / live.sum()
                u[live] = 1 - r if flip else r
            u[m & (v <= -4)] = 1.0
        u = 0.8 * u
...
NOTE = {"oslo": "colour: log10(1 - c) on [-4, 0] (Crameri oslo; dark = the pair has merged, light = uncorrelated)",
        "cyanotype": "colour: log10(1 - c) on [-4, 0] (cyanotype ramp; deep blue = merged, paper = uncorrelated)",
        "spectral": "declared Spectral split: purple half where measured chi_1 < 1, red half where chi_1 > 1; shade = per-side rank of log10(1 - c), dark where the pair is least settled"}
...
def frame(style, l):
    ...
    for i, (c, lab) in enumerate(((cAB, "two unrelated inputs (c0 = 0): do they merge?"),
        ...
        rgb = to_uint8(colour(c[l], style)[::-1])
        tile = Image.fromarray(rgb).resize((pw, ph), Image.NEAREST)
        y0 = 75 + i * (ph + 55)
        img.paste(tile, (60, y0))
        d.text((60, y0 - 32), lab, font=fs, fill=FG[style])
    x = 60 + pw + 30
...
for style in args.styles.split(","):
    ...
    ffmpeg_mp4(fdir, os.path.join(GAL, f"flow_correlation_{style}.mp4"), fps=16)
    ffmpeg_gif(fdir, os.path.join(GAL, f"flow_correlation_{style}.gif"), fps=8, width=960, step=2)
...
fig.text(0.04, 0.95, f"Correlation flow, measured (tanh, N = {N}, {K} nets per pixel): unrelated inputs (top), near-identical inputs (bottom)",
...
for r, c in enumerate((cAB, cAC)):
    for j, l in enumerate(ls_):
        ...
        ax.imshow(np.log10(np.clip(1 - c[l], 1e-4, 1)), origin="lower", cmap="cmc.oslo", vmin=-4, vmax=0, aspect="auto", interpolation="nearest")
        ax.set_xticks([]); ax.set_yticks([])
        ax.set_title(f"l = {l}", fontsize=12, color=INK)
fig.text(0.04, 0.035, "Each tile: the whole plane sigma_w^2 in [0.5, 4.5] (horizontal) x sigma_b^2 in [0, 2] (vertical).\ncolour = log10(1 - correlation) on [-4, 0] (oslo: dark = the two inputs have merged, light = uncorrelated).",
