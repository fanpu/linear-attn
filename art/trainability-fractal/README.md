...
| deep hero, same window, fused engine | 2048² (16 bands) | 500 | float64 | 2 391 s (0.66 h, idle GB10) |
...
**Fused engine** (`cuda/tfkernel.cu`, `tfast.py`, from 2026-09-20). The batched engine above spends its time moving activations: at $P=32\,768$ each forward GEMM writes a 1.14 GB float64 tensor, 500 times per chunk. The replacement gives **one warp per pixel and keeps the whole 500-step loop on-chip**: $X$ (272×16 float64, 34 KB, and identical for every pixel) is staged once per thread block into shared memory, $W_0$, $W_1$ and both gradient accumulators live in registers (149 per thread, no spills), and the only global write is the one double of the measure. Lane $16h+e$ owns column $e$ of $W_0$ and walks rows $i \equiv h \pmod 2$, so the two 16×16 products are lane-local and only $\hat y = h W_1/n$ needs a cross-lane reduction. Early exit becomes per-warp, so a diverged pixel simply leaves the loop and no compaction or reshaping happens at all. On the 1024² hero window this runs at **1755 px/s against 451 px/s** for the batched engine on the same idle GPU (597 s instead of 17 757 s). It is 90 % of its own analytic ceiling on this machine, where float64 runs at 2 FMA per SM-clock (243 G-FMA/s, i.e. 486 GFLOP/s, measured) and one float64 `tanh` costs 22 FMA-equivalents.

The arithmetic stays float64 end to end; what changes is summation *order* — a sequential 16-term FMA dot product instead of cuBLAS `dgemm` over $K=16$, and a butterfly instead of torch's reduction over the 16 hidden units. On the 1024² hero window this moves **9 pixel labels out of 1 048 576** (8 of them boundary pixels, one an isolated speck), against **11** moved by nudging both learning rates by one ulp; the trainable fraction changes by $4.8\times10^{-6}$, the boundary-pixel count by 0.014 %, and $D$ over $b=2$–256 is unchanged at $1.674\pm0.035$. On 262 144 all-converged pixels (`band_000` of the 2048² grid) it gives **zero** label changes and a median relative change of the measure of $4\times10^{-10}$. Full numbers in `PERF-BRIEF.md` §14.
...
- **Throughput.** On a shared GB10, float64 ran at 29–200 px/s. Every wall time in the table above carries that contention: the hero window's 17 757 s is 59 px/s, while the same engine on the same window on an *idle* GPU gives 451 px/s. This is also the whole of the "all-converged bands are 5× faster" puzzle — idle and like for like, a mixed band is 2× *faster* than an all-converged one, because early exit works (`PERF-BRIEF.md` §11). The fused engine below then took the hero window to 1755 px/s. The half-decade sequence was switched to decade steps after $10^{2.5}$ to reach $10^{6.5}$ in the available time. Minibatch-16 and hillshaded relief were built and tested at 128² but not rendered at gallery scale.
