...
from render_common import SERIES, slog, load_exact, exposure_profile, hexrgb, GAL
...
def barcode(style):
    ...
    fig.text(0.08, 30 / Ht, f'bars = exact eigenvalues λ ≥ 3e-4, symlog position (τ=1e-4); digits: C, outlier count, P',
             fontsize=12, color=ink, family='Nimbus Mono PS', alpha=0.7)
...
def gel():
    ...
    for i, s in enumerate(specs):
        expo, _ = exposure_profile(s['H_eig'], lo, hi, Hpx, sigma_px=2.2, d0=0.9)
        ...
        fig.text((xL + lanew / 2) / Wpx, 200 / Ht, f'{int(s["count_H"][0])}', ha='center', fontsize=26,
    ...
    ax.axis('off')
    fig.text(0.5, 110 / Ht, 'lanes: exact Hessian spectra, band = eigenvalue (symlog); number = bands above the widest gap',
