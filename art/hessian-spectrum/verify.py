...
import json, os
import numpy as np
from render_common import SERIES, load_exact, load_lanczos, CACHE
...
for src, loader, key in (('exact', load_exact, 'H_eig'), ('lanczos', load_lanczos, 'H_ritz')):
    for C in SERIES:
        ...
        k = int(k)
        lam = np.sort(d[key])[::-1]
        r = dict(src=src, C=C, P=int(d['P']), N=int(d['N']), loss=float(d['loss']), count=k, gap=float(g),
...
print('| route | C | P | train loss (subset) | outliers k | k = C-1? | gap λk/λk+1 | runner-up gap | count on G | '
      'overlap top-k with span{d_c} | next C | chance C/P |')
print('|---|---|---|---|---|---|---|---|---|---|---|---|')
for r in rows:
    print(f"| {r['src']} | {r['C']} | {r['P']:,} | {r['loss']:.4f} | {r['count']} | {'yes' if r['count'] == r['C'] - 1 else 'no'} "
