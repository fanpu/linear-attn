...
def main():
    for f in sorted(glob.glob('cache/rps_*.npz')):
        ...
        fig.text(0.11, 0.933, f'Largest Lyapunov exponent; fixed {d["fixed"]}', fontsize=11.5, color=INK)
        cap = (f'Q′ = (1−α) Q + β·A(ε) y,  x = softmax(Q), both players (Galla & Farmer 2013 learning rule; SAF 2002 payoffs). '
               ...
               f'λ = {THR:g} (finite-time noise floor for quasi-periodic orbits), each side rank-normalised.')
        fig.text(0.11, 0.035, cap, fontsize=9.5, color=INK, linespacing=1.55)
