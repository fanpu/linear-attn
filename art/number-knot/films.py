...
RMS in-plane radius. Frames resume: existing PNGs are skipped.
"""
...
def sweep(G, meta, S, test_layer=None):
    ...
    # frame the central 99 % of all beads over all layers (declared; rare outliers may leave the frame)
    pts = np.concatenate([G[f"sweep_{k}"].reshape(-1, 3) for k in ("measured", "null")])
    lo, hi = np.quantile(pts, 0.005, axis=0), np.quantile(pts, 0.995, axis=0)
    box = np.array([[x, y, z] for x in (lo[0], hi[0]) for y in (lo[1], hi[1]) for z in (lo[2], hi[2])])
    cam = R.shared_camera([dict(beads=box)], W, H, 35, 30, margin=1.02)
    ...
    for i, x in enumerate(seq):
        ...
        R.label(img, [f"OLMo-2 T=100 helix, measured - {info}", "shuffled labels (null)"], H, (225, 225, 225), W).save(f)
    dt = time.time() - t0
    if test_layer is not None:
        return dt, 1
