...
TF_DOC = ("upper 65 % of cmc.oslo (0.35-1.0) over the grid's loss range; opacity = 6 Gaussian shells (width 0.018) at equally spaced "
          "normalised loss levels 0.08-0.88, density D")
...
def canyon_layer(name, chart, cam, depth, clip_y, density):
    g = np.load(os.path.join(M2, f"canyon_{name}.npz"))
    L = g["loss"].astype(np.float64)  # [a, b, c]
    ...
    def opacity(x):
        ...
        for c in SHELLS:
            o = o + torch.exp(-((x - float(c)) / 0.018) ** 2)
        return o
    ...
    step = float(np.min(np.asarray(hi) - np.asarray(lo))) / 400
...
def camera(chart, az, el, w=S, h=S, zoom=1.0):
...
def hero_render(kind, cam, canyon="hero32", label=True, R=0.006, density=None):
...
if args.plate == "hero":
    ...
    save("hero_glow", hero_render("fixed", cam))
    ...
    Pw = chart(C)
    y_cut = float(np.median(Pw[:, 1]))
    img, depth, glow = ribbon_layers(Pw, lam, cam, R=0.0035, clip_y=y_cut)
    ...
    out = annotate(out, [f"steps {t[0]}–{t[-1]} (EoS phase, Stage B) · axis 1 = piecewise oscillation frames (×20), "
                         "pc1 ×1, pc2 ×2 · orthographic",
                         "canyon: loss on the fixed (u_ref, pc1, pc2) slice through θ(3250), 32³ · a fixed axis 1 only "
                         "holds locally (top-3 subspace overlap 0.30 after 250 steps)"], 0.8)
