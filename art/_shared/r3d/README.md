...
| `shade.py` | `lambert(normal, light_dir, ambient=0.2)` · `hemisphere_dirs(n_rays, seed=0) → (n,3)` (cosine-weighted, +z) · `ambient_occlusion(pos, normal, occluded, n_rays=16, radius=1.0, seed=0, bias=1e-4) → (N,)` (1 = open) · `hard_shadow(pos, normal, light_dir, occluded, bias=1e-4, tmax=1e9) → (N,)` (1 = lit, 0 = shadowed or facing away) |
...
- Clip planes in `march_volume` are applied per sample, so a cut face is quantised to `step`. That leaves ≈1–2% moiré at `step=0.01` against `step/4`; lower `step` for hero plates.
...
- **Plaster torus** (`render_iso` + `ambient_occlusion` + `hard_shadow`): a torus floating 0.1 above a plinth. It should show a smooth body, a cast shadow and contact darkening under the ring.
...
**Known open issues** (visible in the plaster null, reported for a fix):
- `ambient_occlusion` uses one direction set for every point, so AO is quantised in `1/n_rays` steps with sharp-edged bands. This shows as rings on the plinth around the torus. Its tangent frame also switches at `|n_x| = 0.9`, leaving a seam.
- `iso_normals` gives points on a face within `h/4` of a box edge the neighbouring box-face normal. Shadow and AO rays from them then start inside the solid, which leaves black dots along box edges.
