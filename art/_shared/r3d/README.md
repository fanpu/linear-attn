...
|---|---|
| `camera.py` | `Camera(eye, target, up=(0,0,1), width=512, height=512, fov_deg=None, ortho_height=2.0)`: `fov_deg=None` is orthographic. Methods `.rays(device, dtype) → o, d (H,W,3)`, `.project(pts, device=None) → (col,row) (N,2), depth (N,)` (float64, on `device`, default the device of `pts`), `.t_to_depth`, `.depth_to_t`, `.pixel_scale(depth=None)` (world units per pixel; a perspective camera needs `depth` and raises `ValueError` without it). · `orbit(target, radius, az_deg, el_deg, **cam_kw) → Camera` · `turntable(n_frames, target, radius, el_deg, az0_deg=0, **cam_kw) → [Camera]` · `stereo_pair(cam, separation) → (left, right)` (parallel axes) · `ray_box(o, d, lo, hi) → tnear, tfar` |
| `grid.py` | `sample_grid(data, lo, hi, pts, mode="linear"\|"nearest", padding="zeros") → (...,)` or `(..., C)` for `(C,D,H,W)` data · `clip_keep(pts, clip) → bool` |
| `volume.py` | `TransferFunction(cmap, vmin, vmax, opacity, density=1.0, n=1024)`: `opacity(x)` gets `x = (v - vmin)/(vmax - vmin)` clamped to [0, 1], and extinction per world unit is `density * opacity(x)`. · `lut_tf(rgb (L,3), sigma (L,))` for integer labels. It is **categorical** (`tf.categorical = True`) and needs `mode="nearest"`: `march_volume`/`render_volume` raise `ValueError` for a categorical TF with `mode="linear"`, because interpolating labels 1\|3 invents a label-2 shell at the boundary · `march_volume(data, lo, hi, o, d, tf, step, *, tmax=None, clip=(), mode="linear", jitter=True, seed=0) → rgb (N,3), alpha (N,)` · `render_volume(data, lo, hi, cam, tf, step, *, depth=None, clip=(), mode="linear", jitter=True, seed=0, device="cpu") → rgb (H,W,3), alpha (H,W)`. `depth` stops rays at an opaque buffer. `jitter` offsets each ray's segment boundaries by a deterministic `u·step` keyed on (ray index, `seed`). Segment lengths still sum exactly to the path, so step-aligned wood-grain and moiré become fine noise; `jitter=False` puts boundaries at `tnear + k·step`. · `over(front_rgb, front_alpha, back_rgb)`. Any `tf(values) → (rgb, sigma)` callable works; set `tf.categorical = True` on your own label TFs to get the same guard. |
...
| `tubes.py` | `sample_polyline(P, spacing) → pts, s` (`s` = fractional vertex index) · `splat_spheres(centers, radius, cam, *, attrs=None, max_radius_px=256) → dict(depth, normal, attr, mask)` (z-buffered sphere impostors; `radius` scalar or per point). Samples are grouped into power-of-two kernel sizes, so one large splat does not inflate every chunk; samples behind the eye or fully off-screen are skipped. A visible sample wider than `max_radius_px` raises `ValueError` (a perspective camera almost touching the curve): move the camera or shrink the radius, or pass `max_radius_px=None` knowingly · `splat_additive(points, cam, *, weight=None, color=None, sigma_px=0.7, depth=None, eps=0.0) → (H,W)` or `(H,W,3)` Gaussian hairlines (premultiplied), depth-tested if `depth` is given · `visible_runs(P, cam, depth, eps) → [array (M,2) pixel xy]` · `write_svg(path, polylines, width, height, stroke="#000000", stroke_width=1.0, background=None)` |
...
- `splat_additive` sums per point. Pass `weight = spacing / cam.pixel_scale()` so line brightness does not depend on the sampling density. That is the orthographic form; for a perspective camera the scale varies per point, so use `weight = spacing / cam.pixel_scale(cam.project(pts)[1])` (`pixel_scale()` without depth raises).
- Tubes run on the points' device: put `centers`/`points`, and any `attrs`, `color` or `depth` you pass, on the same device.
...
- **GPU** for films and big grids: pass `device="cuda"` and put the grid tensors on the GPU too. Only do this inside a job launched with `setsid nohup /home/fzeng/ml/research/art/_shared/gpu1.sh <cmd...> > log 2>&1 < /dev/null &`. Checkpoint frames so a rerun resumes.
  - **Share mode (the default, user-approved 2026-09-15).** gpu1 runs one art job at a time (the rest queue), waits until `MemAvailable` is at least 30 GB (`GPU1_MIN_FREE_GB`) at start, and sets `PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True`. It does **not** wait for `autonomous/`: your job may run alongside an autonomous/ GPU job on the same ~120 GB unified pool, and the memory check happens only once, at start. Budget your job's memory against a concurrent autonomous/ job, not the whole pool.
  - **`GPU1_EXCLUSIVE=1`** restores the old behaviour: it also takes the autonomous/ lock and waits until nvidia-smi shows no other compute process. Use it for timing measurements that must not share the GPU.
  - gpu1 is re-entrant: a `gpu1.sh` call inside a gpu1 job (for example a chain script that wraps each stage) runs directly under the parent's lock. Never run a GPU job outside gpu1.sh.
