...
for t in range(n):
    ...
    S_delta += np.outer(
        K[t], V[t] - S_delta.T @ K[t]
    )  # delta:    S += k (v - S^T k)^T, beta = 1
...
print(
    f"d = {d}, {n} pairs written; largest |k_i . k_j| between distinct keys = {overlap:.2f}"
)
...
print(
    f"mean error: additive {err_add.mean():.2f}, delta {err_delta.mean():.2f}; "
