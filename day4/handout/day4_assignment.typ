...
The accuracy is the first thing you write, because it is the number everything else today is judged on, and the silent failure is easy: count every position instead of the labeled ones, and a model that predicts $-100$ nowhere scores $0.875$ ($448$ of $512$ positions "correct" by never being asked).
...
#figure(
  table(
    ...
    [Arora et al. #cite(<arora2023zoology>, supplement: [§4, Figure 2]) and the sweep file of their repository, `zoology/experiments/paper_configs/iclr24_zoology_figure2/configs.py`],
    ...
    [Linear attention alone struggles to solve associative recall, and recall accuracy trades off against recurrent state size across architectures.],
...
#figure(
  table(
    ...
    [attention], [$64$], [$2$], [$32$], [learned, $512 times 64$], [$2 dot 2 dot 512 dot 64$],
...
#setting("Learning rates", ${10^(-4), 4.6 times 10^(-4), 2.2 times 10^(-3), 10^(-2)}$)[Four points, evenly spaced in $log_10$ from $-4$ to $-2$; Zoology's grid, and the result is the best of the four.]
...
#setting("Heads", $2$)[Yang et al. use two heads for DeltaNet; we use two for every mixer so that the state size per width is the same for the two linear rules. Zoology's attention used one head; with two the attention KV cache is unchanged.]
...
#problem("predictions", "Three numbers before the first run", points: 1, owner: "you")[
  ...
  #parts(
    ...
    [#prediction(
      ...
      definition: [The smallest width in ${64, 128, 256, 512}$ at which the additive rule's best accuracy reaches $0.9$, or "none"; and the same for the delta rule. From the per-mixer-and-width table that `plot.py` prints (its columns are given in Problem (`smoke`) (a)), which covers the `lr` and `scan` stages together.],
...
#problem("state_scan", "Width scan and a second seed: the third statement", points: 1.5, compute: "0.8 GB10 hrs", owner: "you")[
  ...
  #parts(
    ...
    [Pass bar for the pipeline, not for the hypothesis: every run completed with finite losses; every epoch $-1$ test loss within $0.3$ of $9.01$; the attention smoke run exceeded $0.99$. Then the three statements of @tab-known, each given one of four verdicts: reproduced, contradicted, not detected, or not tested at the published configuration. The rules: the first is reproduced if attention's best accuracy at $d = 64$ exceeds $0.99$, and contradicted otherwise. The second is reproduced if the delta rule's best accuracy at $d = 128$ exceeds the additive rule's by more than $0.2$ and by more than twice the largest seed difference of part (c); contradicted if the additive rule's exceeds the delta rule's by that much; and not detected otherwise. The third is reproduced if each linear rule's best accuracy at $d = 512$ exceeds its value at $d = 64$ by more than $0.2$, contradicted if either falls by that much, and not detected otherwise.
...
$ S_t = S_(t-1) + beta_t (v_t - S_(t-1)^top k_t) k_t^top $ <eq-abl-base>
$ S_t = alpha_t S_(t-1) + beta_t (v_t - alpha_t S_(t-1)^top k_t) k_t^top $ <eq-abl-mod>
...
- Counting unlabeled positions in the accuracy. A model that never answers scores $448\/512$ that way. The metric divides by the number of labeled positions and nothing else.
